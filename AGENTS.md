...
- Produce the real measurement, not a proxy that resembles it. Asked to measure, find, or research something, output the actual data, not a plausible-looking substitute that merely shares its shape. If the real measure is out of reach, say so and name exactly what is missing instead of shipping the lookalike.
...
- A surprising result is a real bug until proven otherwise. Never blame the test, fixture, or your own harness without evidence.
