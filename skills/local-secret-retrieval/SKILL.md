...
- Internal failure pattern: remote-shell GitHub auth failed while the Mac-side GUI/session still had usable credentials, so the correct move was to prove the working surface and continue from there instead of inventing a branch-transfer workaround.
