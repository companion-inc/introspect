...
description: How to create, update, prune, and validate agent skills as scoped procedural memory instead of bloating AGENTS.md. Use whenever the reflector is considering skill_new or skill_update, when adding a reusable workflow from an observed failure, or when deciding whether a lesson belongs in a skill versus the global prompt.
---
...
Create or update a skill only when the lesson is reusable for a class of future tasks and too specific for always-loaded AGENTS.md. If the lesson must apply across nearly every task, edit AGENTS.md instead. If it is one-off anger, casual profanity, or frustration about an external system, make no change.
...
3. Read the primary source for the skill format or behavior being encoded:
...
4. For domain behavior inside the skill, cite actual domain sources: repo files, docs, logs, APIs, papers, issues, or command output. Do not encode generic model folklore as a rule.
5. If no source supports the lesson, write nothing and log why.
...
- SAGE and SkillOS support the same architectural direction: reusable skills distilled from experience improve later related tasks, and skill curators should insert, update, or delete compact Markdown skills based on downstream feedback.
