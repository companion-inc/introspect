...
- Introspect is currently described as a macOS app and hook engine for improving local Claude/Codex instructions from real trigger signals; its public/private split puts reusable app code in this repo and user-specific profile state under `~/.introspect/profile`. Source: `README.md:3`, `README.md:11`.
...
- Existing repo docs already choose the right layer split: `no_change`, `core_prompt`, `project_prompt`, `profile_memory`, `skill_new`, `skill_update`, `project_skill_new`, `project_skill_update`, and `skill_prune`. Source: `README.md:81-82`, `docs/hermes-self-evolution-review.md:44-52`.
