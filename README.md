...
8. After install, the same scanner runs incrementally, skips Codex control/context records, dedupes by transcript line, queues missed classifier-triggered user prompts, and queues narrow assistant self-boundary refusals with a stable `message_locator` of `transcript_path:line`.
