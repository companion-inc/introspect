...
8. After install, the same scanner runs incrementally, skips Codex control/context records, dedupes by transcript line, and queues any missed classifier-triggered prompts into the same queue with a stable `message_locator` of `transcript_path:line`.
